...
// ── STORE GLOBAL (mémoire partagée) ──────────────────────────────────────────
const STORE = { events: [], orgs: [] };
const ADMIN = { email: "admin@kingdom.cm", pwd: "admin2025" };
...
// ── AUTH ORGANISATEUR ─────────────────────────────────────────────────────────
const MOCK = {}; // email→{pwd,eglise,nom}

function OrgAuth({ onBack, onLogin }) {
  const [tab, setTab] = useState("login");
  const [f, setF] = useState({ email:"", pwd:"", eglise:"", nom:"" });
  ...
  const submit = async () => {
    ...
    if (tab==="register") {
      ...
      if (MOCK[f.email]) { setErr("Email déjà utilisé. Connectez-vous."); setLoading(false); return; }
      MOCK[f.email] = { pwd:f.pwd, eglise:f.eglise, nom:f.nom };
      setOk("Compte créé ! Redirection...");
      setTimeout(()=>onLogin({ email:f.email, eglise:f.eglise, nom:f.nom }), 900);
    } else {
      const u = MOCK[f.email];
      if (!u) { setErr("Aucun compte avec cet email."); setLoading(false); return; }
      if (u.pwd!==f.pwd) { setErr("Mot de passe incorrect."); setLoading(false); return; }
      onLogin({ email:f.email, ...u });
    }
    setLoading(false);
  };
    ...
    <div className="auth-wrap">
      <div className="auth-box">
        ...
        {tab==="register"&&<>
          ...
          <div className="fg"><label className="fl2">Responsable</label><input className="fi" placeholder="Pasteur Jean Mbarga" value={f.nom} onChange={e=>setF({...f,nom:e.target.value})}/></div>
        </>}
...
function OrgDash({ user, onLogout, onSubmit }) {
  const [f, setF] = useState({ eglise:user.eglise||"", type:"", titre:"", dateDebut:"", dateFin:"", heureDebut:"", heureFin:"", lieu:"", ville:"", prix:"", desc:"" });
  ...
  const submit = async () => {
    ...
    setErr(""); setLoading(true); setProg(0);
    for (let i=0;i<=100;i+=8) { await new Promise(r=>setTimeout(r,60)); setProg(Math.min(i,100)); }
    onSubmit({ id:Date.now(), ...f, prix:Number(f.prix)||0, photos, videos, emoji:"✝️", bg:C.purpleD, org:f.eglise, statut:"en_attente", email:user.email, soumis:new Date().toLocaleDateString("fr-FR") });
    setLoading(false); setDone(true);
  };

  if (done) return (
    <div>
      <div className="dash-head"><div><div className="dash-name">{user.eglise}</div></div><button className="btn-logout" onClick={onLogout}>Déconnexion</button></div>
      <div className="section" style={{ textAlign:"center", paddingTop:60 }}>
        ...
        <button className="btn btn-g" onClick={()=>{setDone(false);setF({eglise:user.eglise||"",type:"",titre:"",dateDebut:"",dateFin:"",heureDebut:"",heureFin:"",lieu:"",ville:"",prix:"",desc:""});setPhotos([]);setVideos([]);setProg(0);}}>
  ...
  return (
    <div>
      <div className="dash-head">
        <div><div className="dash-name">{user.eglise}</div><div className="dash-sub">{user.email}</div></div>
...
function AdminAuth({ onLogin, onBack }) {
  ...
  const submit = async () => {
    ...
    if (f.email===ADMIN.email&&f.pwd===ADMIN.pwd) onLogin();
  ...
  return (
    <div className="auth-wrap" style={{ background:`linear-gradient(135deg,#08050f,${C.purpleD})` }}>
      <div className="auth-box">
        ...
        <div style={{ fontSize:11, color:C.slateL, marginBottom:14, textAlign:"center" }}>admin@kingdom.cm · admin2025</div>
...
// ── PANNEAU ADMIN ─────────────────────────────────────────────────────────────
function AdminPanel({ events, orgs, onAction, onLogout }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.ivory }}>
      <div className="dash-head">
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <span style={{ color:"rgba(255,255,255,.4)", fontSize:11 }}>{ADMIN.email}</span>
          <button className="btn-logout" onClick={onLogout}>Déconnexion</button>
        </div>
      </div>
...
export default function App() {
  ...
  const [storeEvs, setStoreEvs] = useState([]);
  const [storeOrgs, setStoreOrgs] = useState([]);

  const allEvents = [...SAMPLE, ...storeEvs.filter(e=>e.statut==="approuve")];
  const filtered = allEvents.filter(e=>
    (villeF==="Toutes"||e.ville===villeF)&&
    (typeF==="Tous"||e.type===typeF)
  );

  const handleSubmit = (ev) => {
    setStoreEvs(p=>[...p,ev]);
    if (user) setStoreOrgs(p=>{
      if (p.find(o=>o.email===user.email)) return p;
      return [...p, { ...user }];
    });
  };

  const handleAction = (id, action) => {
    if (action==="supprimer") { setStoreEvs(p=>p.filter(e=>e.id!==id)); return; }
    setStoreEvs(p=>p.map(e=>e.id===id?{...e,statut:action}:e));
  };

  if (admin) return <><style>{G}</style><AdminPanel events={storeEvs} orgs={storeOrgs} onAction={handleAction} onLogout={()=>{setAdmin(false);setPage("home")}}/></>;
  if (showAdm) return <><style>{G}</style><AdminAuth onLogin={()=>{setAdmin(true);setShowAdm(false)}} onBack={()=>setShowAdm(false)}/></>;
  if (showAuth) return <><style>{G}</style><OrgAuth onBack={()=>setShowAuth(false)} onLogin={u=>{setUser(u);setShowAuth(false);setPage("org")}}/></>;
  ...
  return (
    <>
      ...
      {page==="org"&&(
        <OrgPage user={user} onRequestAuth={()=>setShowAuth(true)} onLogout={()=>{setUser(null);setPage("home")}} onSubmit={handleSubmit}/>
      )}
